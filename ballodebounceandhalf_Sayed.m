function [tout,yout,teout,yeout]= ballodebounceandhalf_Sayed
...
e=0.5;% Modification HW00
B=0.1;% Coefficient of viscous drag, HW00
...
for i = 1:2    % Use a loop to conduct up to 10 bounces
   ...
   % HW00. Stop the simulation at the peak of the second bounce. Doing this
   % by changing the event for the second bounce. Stop if detecting
   % velocity decreases to 0. 
   if i==2
options = odeset('Events',@events_bouncehalf,'OutputFcn',@odeplot,'OutputSel',1,...
   'Refine',refine); 
   end
...
%--------------------------------------------------------------------------
function [value,isterminal,direction] = events_bouncehalf(t,y)
% FUNCTION events(t,y)    Event function for ball flight.
%   value = events(t,y) is a zero-crossing function that triggers an event
%                       that may be used to halt simulation.
%                       value is a vector of functions, the zero-crossing
